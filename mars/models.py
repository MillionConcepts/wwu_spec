...
class FilterSet(models.Model):
    ...
    # TODO: it would be useful to have a reasonable cleaning function at some point
...
class Sample(models.Model):
    ...
    filename = models.CharField("Name of Uploaded File", blank=True, max_length=80)
    ...
    material_class = models.CharField("Material Class", blank=True, max_length=40)
    sample_desc = models.TextField("Sample Description", blank=True)
    sample_id = models.CharField("Sample ID", max_length=40)
    sample_type = models.ForeignKey(
        SampleType, on_delete=models.PROTECT, null=True, verbose_name="Sample Type"
    )
    ...
    def clean(self, *args, **kwargs):
        ...
        try:
            ...
            # this should only happen if someone's made typos in the admin console, not with uploaded CSV
            errors.append(
                "Error: the reflectance values don't appear to be formatted as an array."
            )
            ...
            try:
                ...
                if False in nonzeros:
                    warnings.append(
                        "Warning: there are zero- or negative-valued items in the reflectance data for "
                        + self.sample_id
                ...
                # switch to 2-column matrix, sort, check for correct shape, find min and max reflectance
                if self.reflectance.shape[1] != 2:
                    self.reflectance = self.reflectance.T

                # sorts by returning indices that would sort the wavelength column
                self.reflectance = self.reflectance[self.reflectance[:, 0].argsort()]

                if self.reflectance.shape[1] != 2:
                    errors.append(
                        "Error: the reflectance data doesn't appear to be properly organized into two columns."
                    )
    ...
    def save(self, *args, **kwargs):
        ...
        if uploaded:
            if self.sample_id in [sample.sample_id for sample in Sample.objects.all()]:
                for refl_array in Sample.objects.filter(
                    ...
                    if self.reflectance == refl_array["reflectance"]:
                        raise ValueError(
                            "The sample "
                            + self.sample_id
                            + " appears to already be in the database. If you're"
                            + " sure this is a unique sample, please give it a new"
                            + " ID. If you're trying to correct a previously"
                            + " uploaded sample, please contact the site"
                            + " administrator to delete your previous uploads."
                        )
                else:
        ...
        if self.image:
            if isinstance(self.image, PIL.ImageFile.ImageFile):
                ...
                raise ValueError(
                    "Associated image field must be a PIL.ImageFile.ImageFile object or a pathname."
                )
        ...
        for filterset in FilterSet.objects.all():
            ...
            sims[filterset.name+'_no_illumination'] = simulate_spectrum(self,filterset,illuminated=False)
        for sim in sims:
    ...
    def as_dict(self):
        self_dict = {}
        for field in self._meta.fields:
            self_dict = dict(**self_dict, **{field.name: getattr(self, field.name)})
        return self_dict

    def write_sims(self):
        sims = dict(json.loads(self.simulated_spectra))
        frames = [pd.read_json(sims[key]).reindex(columns=[key+' filter','wavelength','response']) for key in sims]
        return [frame.to_csv() for frame in frames]

    def as_json(self, simulate=False):
        json_dict = {}
        for field in self._meta.fields:
            if getattr(self, field.name):
                if field.name == "reflectance":
                    json_dict = dict(
                        **json_dict, **{"reflectance": dict(literal_eval(self.reflectance))}
                    )
                    ...
                    for filterset in sims:
                        name = filterset
                        # 'astype(float)'' is added because json will not treat numpy.int64
                        # as an int or float, unlike its treatment of numpy.float64,
                        # which causes problems for samples that have reflectance ranges
                        # that lie totally outside of a filterset's range
                        spectrum = dict(pd.read_json(sims[filterset]).drop(columns="filter").values.astype(float))
